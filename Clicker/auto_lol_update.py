...
SKILL_UPGRADE_ORDER = ['d', 'w', 'd', 'a', 'd', 'space', 'd', 'w', 'd', 'w', 'space', 'w', 'a', 'a', 'a', 'space', 'a',
                       'a']
...
game_state = {
    ...
    # 【新增】记录商店购买状态，防止在泉水里无限买东西
    'has_shopped_this_visit': False,

    # 【新增】紧急技能的最后释放时间，用于替代 ACTION_CONFIG 的循环冷却
    'last_cast': {
        'e': 0.0,
        'space': 0.0
    }
...
# 【修改】已将 'e' 和 'space' 从定时循环中移除，改为由视觉线程瞬间触发
ACTION_CONFIG = {
    'w': {'key': 'w', 'start': 25.0, 'end': 5.0, 'delay': 0.0, 'condition': 'none'},
    'd': {'key': 'd', 'start': 20.0, 'end': 3.0, 'delay': 0.0, 'condition': 'low_health'},
...
def visual_monitor_thread():
    ...
    # 商店图标高亮阈值 (通常亮起时亮度在100以上，灰暗时在80以下，你可根据输出微调)
    ...
    while True:
        if game_state['is_running'] and game_state['window_moved']:
            try:
                ...
                # 【新增】商城图标区域 (X:611~694, Y:746~762 => width:83, height:16)
                shop_region = {
                ...
                with mss.MSS() as sct:
                    ...
                    if game_state['current_level'] > 0:
                        ...
                        hp_center_x = client_point[0] + TEAMMATE_X_LIST[current_teammate_idx]

                        # 【修改】X坐标向右平移10像素 (left: hp_center_x - 5 + 10 = hp_center_x + 5)
                        health_region = {
                        ...
                        if current_time - last_print_time > 5.0:
                            ...
                            # 调试输出商店亮度：print(f"[Debug] 商店亮度:{shop_mean:.1f}")
                        ...
                        if not is_attached:
                            ...
                            if game_state['is_paused'] and not is_in_base:
...
def action_worker(action_name, config, start_offset):
    ...
    while True:
        ...
        if game_state['is_running'] and game_state['start_time'] is not None and not is_paused_now:
            current_time = time.time()
